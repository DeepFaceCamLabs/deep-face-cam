...
def has_audio_stream(target_path: str) -> bool:
    command = [
        "ffprobe",
        "-v",
        "error",
...
def restore_audio(target_path: str, output_path: str) -> None:
    temp_output_path = get_temp_output_path(target_path)
    if not has_audio_stream(target_path):
...
def get_temp_directory_path(target_path: str) -> str:
    safe_root = os.path.abspath(TEMP_DIR)
    Path(safe_root).mkdir(parents=True, exist_ok=True)
    target_name, _ = os.path.splitext(os.path.basename(target_path))
    digest = hashlib.sha1(os.path.abspath(target_path).encode("utf-8")).hexdigest()[:12]
    return os.path.join(safe_root, f"{target_name}-{digest}")
...
def clean_temp(target_path: str) -> None:
    ...
    if parent_directory_path != os.path.abspath(TEMP_DIR) and os.path.exists(parent_directory_path) and not os.listdir(parent_directory_path):
