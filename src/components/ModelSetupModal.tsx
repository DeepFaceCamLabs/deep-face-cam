...
export function ModelSetupModal({
  ...
  const missingModels = requiredModels.filter((model) => !model.present);
  const missingBytes = missingModels.reduce(
  ...
  const download = async () => {
    ...
    try {
      const result = await rpc.downloadModels(undefined, true);
      onStatusChange(result.status);
      if (!result.ok || result.status.missing_required.length > 0) {
